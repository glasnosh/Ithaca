...
public class tablero extends JPanel implements MouseListener {
    ...
    public tablero(malla mimalla,Object[][] elementos_tabla){
...
(new blanquear_sentencias(elementos_tabla)).run();
    ...
    public int insertar_aleatorio(int forma, int color, int tamanho){	//Lo aleatorio es la posición donde se inserta el elemento
    	int result = 0;
	if (mimalla.numElementos() >= 15) result = 1;	//máximo 15 elementos en la malla
	else{
		(new blanquear_sentencias(elementos_tabla)).run();	//al modificar la malla los análisis no valen, así que todo en blanco
		//buscar hueco libre
		...
		while (hueco_encontrado != true){
			...
			if ( mimalla.devolverCasilla(x,y).ocupada() == false ) {
				mimalla.insertar(x,y,forma,tamanho,color);
