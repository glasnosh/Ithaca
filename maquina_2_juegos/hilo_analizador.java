...
public class hilo_analizador extends Thread{
	...
	public hilo_analizador(malla mimalla, JTextField area_texto, int tipo_analisis){	// La sentencia se toma con todo el 
		...
		cadena = (new miniconversor()).convertir_visual_interno(_cadena);	//el miniconversor transforma todo al lenguaje interno del programa
	}//constructor
	...
	public void run(){
		if (area_texto.getText().compareTo("") == 0){	//Si el area esta vacía, simplemente inicializarla en colores
			...
			if (mimalla.numElementos() != 0){	//si la malla tiene elementos del usuario
				...
				}catch (Exception error){System.out.println("PARSER: Error de parseo no especificado");}
