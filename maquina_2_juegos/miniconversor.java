/*
 *  "miniconversor.java"
 * 	Basado en la clase "miniconversor" de ITHACA, pero también hace la conversión simétrica del lenguaje de la máquina
 *	 al fisual.
 *	Transforma los caracteres que usa la interfaz gráfica en el lenguaje propio de los parsers.
 * 	 vg.-	masPequenho  -->	MASPEQUENHO	-->	masPequenho
 *			
...
/*
	...
	public String convertir_visual_interno(String cadena)	//pequeño --> PEQUENHO
	public String convertir_interno_visual(String cadena)	//PEQUENHO --> pequeño
*/
...
class miniconversor{
	...
	public String convertir_interno_visual(String cadena){
		...
		*/
		...
		// Los siguientes 2 bucles "for" sirven para eliminar los paréntesis de T() para que aparezca como T
		// IDEM para la función falsedad. No se pq la función replaceAll se los salta.
		
		for (int i = 0; i< cadena.length(); i++){
			if (  ( cadena.charAt(i) == _T) && (i < cadena.length()-1) ){
			if ( (cadena.charAt(i+1) == '(') && (cadena.charAt(i+2) == ')') )
				cadena = cadena.substring(0,i+1).concat( cadena.substring(i+3, cadena.length()));	
			}
		}
		
		for (int i = 0; i< cadena.length(); i++){
			if (  ( cadena.charAt(i) == _nT) && (i < cadena.length()-1) ){
			if ( (cadena.charAt(i+1) == '(') && (cadena.charAt(i+2) == ')') )
				cadena = cadena.substring(0,i+1).concat( cadena.substring(i+3, cadena.length()));	
			}
		}
		...
		cadena = cadena.replaceAll("GRANDE","grande");
		cadena = cadena.replaceAll("PEQUENHO","pequeño");
		cadena = cadena.replaceAll("ROJO","rojo");
		cadena = cadena.replaceAll("AZUL","azul");
		cadena = cadena.replaceAll("AMARILLO","amarillo");
		cadena = cadena.replaceAll("ESFERA","esfera");
		...
		cadena = cadena.replaceAll("MASGRANDE","masGrande");
		cadena = cadena.replaceAll("MASPEQUENHO","masPequeño");
		cadena = cadena.replaceAll("MISMAFORMA","mismaForma");
		cadena = cadena.replaceAll("MISMOCOLOR","mismoColor");
		cadena = cadena.replaceAll("MISMOTAMANHO","mismoTamaño");
		cadena = cadena.replaceAll("MISMAFILA","mismaFila");
		cadena = cadena.replaceAll("MISMACOLUMNA","mismaColumna");
		cadena = cadena.replaceAll("IZQUIERDA","izquierda");
		cadena = cadena.replaceAll("DERECHA","derecha");
